...
fn main() {
    let guess_list: [&str; 5] = ["mango", "apple", "banana", "grapes", "orange"];

    let mut rng = rand::rng();

    loop {
        if let Some(random) = guess_list.choose(&mut rng) {
            println!("Random Fruit: {}", random);
            let mut input = String::new();
            print!("Enter a fruit name: ");
            io::stdout().flush().unwrap();
            io::stdin().read_line(&mut input).expect("fail to readline");
            if input.trim().to_lowercase() == *random {
                println!("Congratulations! You guessed the correct fruit.");
                break;
            } else {
